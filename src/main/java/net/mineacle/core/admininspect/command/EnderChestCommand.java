...
public final class EnderChestCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!viewer.hasPermission("mineacleadmin.echest")) {
        ...
        if (args.length < 1) {
            viewer.sendMessage(TextColor.color("&cUsage: /echest <player>"));
            SoundService.guiError(viewer, core);
            return true;
        }
        ...
        if (target.equals(viewer) && !viewer.hasPermission("mineacleadmin.echest.self")) {
            viewer.sendMessage(TextColor.color("&cYou cannot inspect your own ender chest"));
            SoundService.guiError(viewer, core);
            return true;
        }

        if (target.getGameMode() == GameMode.SPECTATOR) {
            viewer.sendMessage(TextColor.color("&cThat player's ender chest cannot be inspected right now"));
            SoundService.guiError(viewer, core);
            return true;
        }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (!(sender instanceof Player player) || !player.hasPermission("mineacleadmin.echest")) {
            return List.of();
        }
